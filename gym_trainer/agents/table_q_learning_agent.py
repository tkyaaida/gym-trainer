...
class TableQLearningAgent:
    def __init__(self, dim_obs, dim_action, obs_min, obs_max, n_disc,
        ...
        Args:
            ...
            n_disc (int): number of discretization parameter. Each dimension is discretized with 2*n_disc-1 tile
    ...
    def step(self, obs, action, obs_next, reward, i_episode):
        ...
        self.q_values[state, action] += \
            self.lr * (reward + self.gamma * self._q_max(state_next) - self.q_values[state, action])

        # choose action
        action_next = self.get_action(state, i_episode)
        return action_next
